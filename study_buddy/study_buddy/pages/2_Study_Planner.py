    ...
    with st.form("add_task"):
        ...
        if st.form_submit_button("Add task"):
            if not title.strip():
                ...
                tasks = load("tasks")
                tasks.append({
                    "id": next_id(tasks), "user_id": u["id"],
                    "title": title.strip(), "subject": subject.strip(),
                    "deadline": deadline.isoformat(), "priority": priority,
                    "done": False,
                })
                save("tasks", tasks)
...
view = tasks
if subj_filter:
    view = [t for t in view if subj_filter.lower() in (t.get("subject") or "").lower()]
    ...
    if cur:
        with st.form("edit_form"):
            ...
            if st.form_submit_button("Save"):
                ...
                for x in all_tasks:
                    if x["id"] == tid:
                        x.update({"title": title, "subject": subject, "priority": priority})
                save("tasks", all_tasks)
                del st.session_state["edit_task"]; st.rerun()
