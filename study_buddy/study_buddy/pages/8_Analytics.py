...
if tasks:
    ...
    c1.plotly_chart(px.pie(pie, names="status", values="count", title="Tasks status"),
                    use_container_width=True)
    subj = df.groupby(df["subject"].fillna("Unknown")).size().reset_index(name="count")
    c2.plotly_chart(px.bar(subj, x="subject", y="count", title="Tasks by subject"),
                    use_container_width=True)
else:
...
if quizzes:
    ...
    st.plotly_chart(px.line(df, x="at", y="pct", markers=True, title="Quiz score trend (%)"),
                    use_container_width=True)
...
if revs:
    ...
    st.plotly_chart(px.bar(rs, x="status", y="count", title="Revision status"),
                    use_container_width=True)
